...
SUPPORT_SCREENLY = SUPPORT_PLAY_MEDIA | SUPPORT_NEXT_TRACK | SUPPORT_PREVIOUS_TRACK
...
async def async_setup_platform(
    ...
    screenly = ScreenlyDevice(async_get_clientsession(
        hass, verify_ssl=False), name, host, port, ssl, timeout)
...
class ScreenlyDevice(MediaPlayerDevice):
    ...
    def __init__(self, websession, name, host, port, encryption, timeout):
    ...
    async def async_play_media(self, media_type, media_id, **kwargs):
        ...
        if media_type in [MEDIA_TYPE_IMAGE, MEDIA_TYPE_VIDEO, MEDIA_TYPE_URL]:
            response = await self._screenly.switch_asset(media_id)
